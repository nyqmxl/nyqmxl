...
class Api:
    ...
    async def call(Class=None, List=list()):
            ...
            for f1 in Dict:
                try:
                    ...
                    if callable(swap_exec) and "params" in Dict[f1]:
                        if Dict[f1]["params"]:
                            swap_call = await swap_exec(**Dict[f1]["params"].copy())
                        else:
                            swap_call = await swap_exec()
    ...
    async def delay(self, **kwargs):
        timeout = kwargs.get("delay", 0.0)
        await asyncio.sleep(timeout)
        return timeout

    async def demo(self, **kwargs):
        print(**kwargs)

    async def __browser__(self, **kwargs):
        return Camoufox(
            os="windows",
            screen=Screen(
                **kwargs.get(
    ...
    async def browser(self, **kwargs):
        from time import time
        swap_exec = None
        self.pointer_browser = (await self.__browser__(**self.kwargs).start()).pages[0]
        for f1 in kwargs.copy():
            try:
                ...
                if "params" in kwargs[f1]:
                    ...
                    if kwargs[f1]["params"]:
                        swap_call = await swap_exec(**kwargs[f1]["params"])
                    else:
                        swap_call = await swap_exec()
                    swap_exec = True
            except BaseException as e:
    ...
    async def launch(self):
        self.pointer_browser = await self.__browser__(**self.kwargs)
        self.pointer_browser = await self.pointer_browser.start()
        self.pointer_browser = await self.pointer_browser.pages[0]

    async def close(self):
        await self.pointer_browser.close()

    async def goto(self, **kwargs):
        print(kwargs)
        await self.pointer_browser.goto(**kwargs)
        return kwargs

    async def test(self):
        page = self.pointer_browser  # 获取默认打开的唯一标签页
        
        print("kwargs")
        data = await getattr(page, "goto", None)(**{"url": "https://www.baidu.com"})
        data = await getattr(page, "title", None)()
...
async def demo():
    ...
    launcher = Api()
    if False:
        await launcher.launch()  # 明确启动浏览器
        await launcher.goto(**{"url": "https://www.devicescan.net/zh"})  # 访问页面
    else:
        if True:
            launcher_data = await launcher.call(
                [
                    {
                        "launch": {"params": None},
                    ...
                    {"delay": {"params": {"timeout": 3}}},  # bug fix
                    {
                        "launch": {"params": None},
